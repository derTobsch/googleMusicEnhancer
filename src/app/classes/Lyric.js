    ...
    function findBy(strategy, parameter) {
        ...
        if (persistLyric) {
            ...
            strategy.execute(parameter.artist, parameter.title,
                ...
                function (name, url) {
                    var googleSearchString = encodeURI(baseGoogleUrl + '?q=lyrics+' + parameter.artist + '+"' + parameter.title + '"#q=lyrics+' + parameter.artist + '+"' + parameter.title);
                    ...
                    $('#lyrics-body').html('')
                        .append(
                            ...
                            Build.div({attr: {class: 'lyrics-not-found-link'}}).append(Build.link({attr: {href: googleSearchString}, text: 'Search for \'' + parameter.artist + ' - ' + parameter.title + '\''}))
                        ).append(
